...
Упрощённая инициализация, Markdown вместо HTML, без os.environ.
"""
...
async def process_subject_choice(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        "одним сообщением. Например: _Мордкович параграф 1 номер 1_",
        parse_mode="Markdown",
    )
    await callback.answer()
...
async def search_gdz(query: str) -> str | None:
    """Запрос к Google Custom Search API. Возвращает ссылку на первый результат или None."""
...
async def process_task_query(message: Message, state: FSMContext):
    ...
    if link:
        ...
        await message.answer("Ничего не найдено, попробуй написать проще 🙁")

    await state.clear()


async def main():
    # Максимально простая инициализация — без DefaultBotProperties
