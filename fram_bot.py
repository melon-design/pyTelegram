...
POLICY_TEXT = (
    ...
    "1. Валюта Fram является внутренней виртуальной валютой бота и не имеет "
    "реальной денежной стоимости.\n"
    "2. Администрация бота не несёт ответственности за утрату баланса, "
    "ошибочные переводы, действия третьих лиц, а также за любые "
    "последствия использования бота.\n"
    "3. Бонус «🎁 Бонус» можно активировать только после решения "
    ...
    "уведомления.\n"
    "5. Запрещены попытки взлома, эксплуатации ошибок и мошенничество — "
...
async def cmd_start(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        f"Добро пожаловать в <b>Fram</b> — валюту с экосистемой!\n\n"
...
async def bonus_start(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer(
        f"🎁 Чтобы получить бонус {BONUS_AMOUNT} Fram, решите пример:\n\n"
        f"<b>{a} {op} {b} = ?</b>\n\n"
...
async def bonus_check(message: Message, state: FSMContext) -> None:
    ...
    if user_answer == correct_answer:
        if has_claimed_bonus(message.from_user.id):
            ...
            await message.answer(
                f"✅ Правильно! Вам начислено {BONUS_AMOUNT} Fram.\n\n"
...
async def transfer_start(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        ...
        "Узнать свой ID можно, например, через бота @userinfobot."
    )
...
async def transfer_confirm(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if success:
        ...
        await callback.message.edit_text(
            ...
            f"Отправлено: {amount} Fram → ID {recipient_id}\n"
            f"Ваш новый баланс: <b>{new_balance} Fram</b>"
        )
        try:
            await callback.bot.send_message(
                recipient_id,
                f"💠 Вам поступил перевод {amount} Fram от пользователя "
