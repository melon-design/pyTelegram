...
async def cmd_start(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        ...
        "━━━━━━━━━━━━━━━━\n\n"
        f"{BTN_TRANSFER} — отправить Fram другому пользователю\n"
        f"{BTN_PROFILE} — ваш баланс и бонус\n"
...
async def show_profile(message: Message, state: FSMContext) -> None:
    ...
    if bonus_available:
        ...
        text += f"\n\n⏳ Следующий бонус будет доступен через {format_timedelta(remaining)}."
...
async def bonus_start(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if not available:
        await callback.answer(
            f"Следующий бонус через {format_timedelta(remaining)}.", show_alert=True
        )
        return
